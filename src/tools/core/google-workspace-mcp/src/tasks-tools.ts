...
export async function tasksListTasks(args: { tasklist: string, showCompleted?: boolean, maxResults?: number }) {
  const params: any = { 
...
export async function tasksCreateTask(args: { tasklist: string, title: string, notes?: string, due?: string }) {
  const params = { tasklist: args.tasklist };
  const json: any = { title: args.title };
  if (args.notes) json.notes = args.notes;
  if (args.due) json.due = args.due;
...
export async function tasksGetTask(args: { tasklist: string, task: string }) {
...
export async function tasksUpdateTask(args: { tasklist: string, task: string, title?: string, notes?: string, status?: string, due?: string }) {
  const params = { tasklist: args.tasklist, task: args.task };
  const json: any = { id: args.task };
...
export async function tasksDeleteTask(args: { tasklist: string, task: string }) {
  const params = { tasklist: args.tasklist, task: args.task };
